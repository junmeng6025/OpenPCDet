...
def parse_config():
    ...
    parser.add_argument('--cfg_file', type=str, default="cfgs/kitti_models/PartA2_relation_mamba.yaml", help='specify the config for training')
...
def floptest():
    ...
    date_tag = '%s'% datetime.datetime.now().strftime('%Y%m%d-%H%M%S') if args.date_tag is None else args.date_tag
    ...
    # test flops
    model.cuda()
    test_data = load_pkl('batch_dict_flop_bs2')
    # test_data.cuda()
    macs, params = profile(model, inputs=(test_data, ))
    print("=> FLOPs and Params of model %s"%(args.cfg_file))
    print(macs, params)
    print("FLOPs(MACs): %.2fG\nPARAMs: %.2fM" % (macs/1000000000, params/1000000))
