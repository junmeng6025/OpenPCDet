...
def make_patch_embed(in_chans, embed_dim, kernel_size, stride, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
...
class BaseBEVBackboneMamba(nn.Module):
    def __init__(self, model_cfg, input_channels):
        ...
        for idx in range(num_levels):
            ...
            if len(upsample_strides) > 0:
                ...
                if stride > 1 or (stride == 1 and not self.model_cfg.get('USE_VSSBLOCK_FOR_NO_STRIDE', False)):
                    ...
                    self.deblocks.append(nn.Sequential(
                        nn.Conv2d(
                            num_filters[idx], num_upsample_filters[idx], upsample_strides[idx],
                            stride=layer_strides[idx], padding=0, bias=False
                        ),
                        Permute(0, 2, 3, 1),
                        VSSBlock(num_upsample_filters[idx]),
                        Permute(0, 3, 1, 2)
                    ))
    ...
    def forward(self, data_dict):
        ...
        if len(ups) > 1:
            x = torch.cat(ups, dim=1)  # (B, 128, H_in/str, W_in/str)  pp: (B, 128, 248, 216) / mamba:() / pvrcnn: (B, 512, 200, 176)
        elif len(ups) == 1:
            x = ups[0]
