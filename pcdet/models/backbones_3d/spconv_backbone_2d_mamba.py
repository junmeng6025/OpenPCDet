...
def post_act_block_mamba(out_channels):
    m = VSSBlock(hidden_dim=out_channels, drop_path=0.1)
    return m


class PillarBackBoneMamba(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        block = post_act_block_mamba
        dense_block = post_act_block_dense

        self.patch_embed = make_patch_embed(
            in_chans=input_channels, 
            embed_dim=out_channels, 
            patch_size=4, 
            patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False)
